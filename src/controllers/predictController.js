...
const predictWaste = async (req, res) => {
  ...
  try {
    ...
    if (!recommendation.length) {
      return res
        .status(404)
        .json({ error: "No recommendation found for this category" });
    }
...
const getPredictionHistory = async (req, res, next) => {
  ...
  try {
    ...
    error.statusCode = 500;
    error.message = error.message || "Error retrieving prediction history";
    error.status = false;
    next(error);
  }
};
...
const getPrediction = async (req, res) => {
  ...
  try {
    ...
    error.message = error.message || "failed to retrieve prediction";
    error.status = false;
    error.errors = error;
    next(error);
  }
